...
**Stack:** Ubuntu (VirtualBox, bridged adapter) · Docker & Docker Compose · Nginx · Flask · MySQL 8 · Prometheus · Node Exporter · Bash · cron

Nginx (80) is published to the host's network interface and reachable from any device on the network. Prometheus (9090) is published to `127.0.0.1`
only, reachable from inside the VM, not from the network (see [Security notes](#7-security-notes) for why). The app, database, and node-exporter are 
only reachable from other containers on the `devops-network` bridge network.
...
- An SSH key pair generated on your **host machine** (not the VM):
  ...
  ```
  Keep the private key on your host, you'll paste the public key onto the VM during setup.

---

## 2. Setup

### Step 1 - Provision & harden the host

Create a dedicated user and add it to the `sudo` group:
```bash
sudo adduser admin
sudo usermod -aG sudo admin
```

Install your SSH public key for `admin`:
```bash
sudo -iu admin
...
```
If it still shows port `22`, the cause is Ubuntu's **systemd socket activation** for SSH — a separate `ssh.socket` unit binds to port 22 independently 
and never reads the `Port` directive in `sshd_config`.
Confirm with:
```bash
...
```
Log out and back in (`exit`, then reconnect) so the group membership takes effect — group changes only apply to new sessions. 
Confirm with:
```bash
...
### Step 3 - Bring up the application stack
```bash
git clone https://github.com/AnubhavPradhan/HardenedStack-Multi-Container-Deployment-and-Monitoring.git
cd HardenedStack-Multi-Container-Deployment-and-Monitoring
...
### Step 4 - Install the health check script + cron
```bash
...
### Step 5 - Install the backup script + cron
```bash
sudo cp scripts/db_backup.sh /opt/scripts/
sudo chmod +x /opt/scripts/db_backup.sh
sudo /opt/scripts/db_backup.sh            # run once manually to confirm it works
ls -lh /var/backups/db/
```
Schedule it daily at 02:00 AM, add this line to the root crontab
(`sudo crontab -e`):
```
...
Prometheus scrapes Node Exporter every 15 seconds (config in `monitoring/prometheus.yml`) for host-level metrics: CPU, memory, disk, network, with 
no extra setup on the host itself.

**Access is restricted to the VM itself** (see Security notes below for why), 
view the dashboard from inside the VM, not from the host or another device on the network:
...
Port 9090 is bound to `127.0.0.1` only in `docker-compose.yml` - unlike Nginx's port 80, it is intentionally **not** reachable from the network.

---

## 5. Database backup & restore

Backups are stored at `/var/backups/db/db_backup_YYYYMMDD.sql.gz`, retained for 7 days (older files are pruned automatically by the script).
...
```
`$(date +%Y%m%d)` assumes you're restoring today's backup — replace it with an explicit date (e.g. `db_backup_20260911.sql.gz`) when restoring 
an older one.
...
- `ufw` default-denies everything except 2222 (SSH), 80 (HTTP), 443 (HTTPS).
  Port 443 is opened ahead of TLS termination. Nginx currently serves
  plain HTTP only.
...
- Database credentials in `docker-compose.yml` and the scripts are
  placeholders for local use, in production these would move to a `.env`
  file (already git-ignored) or a secrets manager.
